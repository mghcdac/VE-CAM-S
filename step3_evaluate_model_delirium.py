...
if __name__=='__main__':
    ...
    Ncv = 5
    random_state = 2020
    with open('results_%s_Nbt0.pickle'%model_type, 'rb') as ff:
        res = pickle.load(ff)
    for k in res:
        exec('%s = res[\'%s\']'%(k,k))

    df_pred_all = pd.read_csv('cv_predictions_%s_Nbt0.csv'%model_type)
    ...
    df_perf_subset.to_csv('perf_subset.csv', index=False)
